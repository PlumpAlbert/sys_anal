        ...
        <tbody>{
          props.nodes.map(node => {
            ...
            for (let i = 0; i < props.links.length; i++) {
              ...
              else if (props.links[i].target === node && props.links[i].twoWay) {
                matches.push((props.links[i].source as GraphNode).id);
              }
            }
            return <tr key={`row_${node.id}`}>
              <th>{node.id}</th>
              {
                props.nodes.map(v => {
                  if (matches.includes(v.id)) return <td key={`${node.id}_${v.id}`}>1</td>;
  ...
  // if (props.type === 'Incidence')
    ...
    <table className='matrix-table'>
      <thead>
        <tr>
          <th> </th>
          {props.links.map(link => <th key={`head_${link.label}`}>{link.label}</th>)}
        </tr>
      </thead>
      <tbody>{
        props.nodes.map(node => {
          ...
          return (
            <tr key={`row_${node.id}`}>
              <th>{node.id}</th>
              {
                content.map((v, i) => <td key={`${node.id}_${i}`}>{v}</td>)
              }
            </tr>
          );
        })
...
const mapS2P: MapStateToProps<IStoreProps, IOwnProps, IGlobalState> = ({ nodes, links }, ownprops) => ({
  nodes,
  links,
  ...ownprops
});
