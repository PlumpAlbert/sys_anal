          ...
          .attr('d', d => {
            ...
            if (!destNode.x
              ...
              return `M${startX},${startY}
               L${destNode.x - (circleRadius + 10) * Math.cos(angle)},${destNode.y - (circleRadius + 10) * Math.sin(angle)}`;
            } else {
    ...
    if (nodeSelection.size() !== nodes.length) {
      ...
      newSelection
        ...
        .on("mouseup", d => {
          ...
          if (createNewLink) {
            this.props.setLink(this.props.links.length, {
              source: this.startNode,
              target: d,
              twoWay: this.startNode === d,
              // @ts-ignore
              label: `e${this.d3state.linkCount++}`
            });
          }
          e.stopPropagation();
          this.startNode = null;
...
const dispatchToProps: MapDispatchToProps<IDispatchToProps, {}> = dispatch => ({
  ...
  setLink: (index, link) => dispatch(updateLink({index, link}))
});
